    ...
    <nav className="bg-indigo-600 text-white p-4">
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-indigo-700 mt-2 p-4 rounded-md">
          {isAuthenticated ? (
            <>
              <Link to="/dashboard" className="block py-2 hover:text-indigo-200">Dashboard</Link>
              <Link to="/profile" className="block py-2 hover:text-indigo-200">Profile</Link>
              <button 
                onClick={handleLogout}
            ...
            <>
              <Link to="/login" className="block py-2 hover:text-indigo-200">Login</Link>
              <Link to="/register" className="block py-2 hover:text-indigo-200">Register</Link>
            </>
          )}
        </div>
      )}
